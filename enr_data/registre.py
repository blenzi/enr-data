...
def load_installations():
    filiere = {
        "solaire photovoltaïque": "Photovoltaïque",
    ...
    installations = (
        gpd.read_file("data/registre.gpkg", layer="registre")
        .to_crs(epsg=4326)
        .assign(
            Filière=lambda x: x["typo"].replace(filiere),
            energie_GWh=lambda x: x["prod_MWh_an"] * 1e-3,
        )
        .rename(columns={"millesime": "annee"})
        .astype({"annee": "int"})
    )
    ...
    columns = [
        ...
        "NOM_REG",
        "EPCI",
        "DEPARTEMENTS_DE_L_EPCI",
        "annee",
        "geometry",
    ]
    return pd.concat(
        [
            installations.loc[installations["Filière"].isin(filieres)],
            load_installations_biogaz(),
        ]
    )[columns]
...
def load_installations_biogaz():
    return (
        gpd.read_file("data/registre.gpkg", layer="installations_biogaz")
        .to_crs(epsg=4326)
        .rename(
            columns={
...
def get_indicateurs_registre(installations):
    ...
    France = get_sum(installations.groupby(["annee", "Filière"])).assign(
        Zone=region_default, TypeZone="Régions"
    )

    ind = [
        get_sum(installations.groupby(["annee", "Filière", column]))
        .rename(columns={column: "Zone"})
        .assign(TypeZone=type_zone)
        for type_zone, column in {
            "Epci": "NOM_EPCI",
...
def get_indicateurs_sdes():
    return (
        pd.read_csv("data/SDES_indicateurs_depts_regions_France.csv")
        .set_index("Zone")
        .drop("Total DOM")
        .reset_index()
        ...
        .pivot_table(
            index=["TypeZone", "Zone", "Filière", "annee"],
            values="valeur",
            columns="indicateur",
        )
        .assign(
            puiss_MW=lambda x: x["Puissance.totale.en.kW"] / 1e3, type_estimation="SDES"
        )
...
def get_indicateurs(installations):
    ...
    ind = indicateurs.set_index(sdes.index.names)
    sdes["energie_GWh"] = ind.loc[sdes.index.intersection(ind.index), "energie_GWh"]
    return (
        pd.concat([indicateurs, sdes.reset_index()])
        .drop_duplicates(["TypeZone", "Zone", "annee", "Filière"], keep="last")
        .astype({"annee": int})
    )
...
if __name__ == "__main__":
    ...
    epcis.loc[multi, "DEPARTEMENTS_DE_L_EPCI"] = epcis.loc[
        multi, "DEPARTEMENTS_DE_L_EPCI"
    ].apply(fcn_epci)
    epcis.explode("DEPARTEMENTS_DE_L_EPCI").drop_duplicates().dropna().to_csv(
        "data/epcis.csv", index=False
    )

    # Export installations and indicateurs
    installations[
        ~installations.geometry.is_empty
        & (installations["annee"] == installations["annee"].max())
    ].to_file(
        "data/app.gpkg", layer="installations"
    )  # TODO: drop columns
